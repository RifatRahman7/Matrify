...
const adminRoutes = [
  ...
  { name: "Approved Premium", path: "/dashboard/approvedPremium", icon: <FcApprove className="text-black dark:text-white w-5 h-5 mr-2" /> },
...
const DashboardLayout = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-green-50 via-white to-blue-50 dark:from-slate-950 dark:via-slate-950 dark:to-slate-950 roboto">
      ...
      {/* Mobile Hamburger */}
      <div className="md:hidden flex justify-between items-center px-4 py-3 border-b bg-white dark:bg-slate-900 shadow">
        <h2 className="font-bold text-lg text-gray-800 dark:text-gray-100">Dashboard</h2>
      ...
      <div className="flex flex-1 max-w-7xl mx-auto w-full py-6 px-2 md:px-6 gap-8">
        {/* Desktop Sidebar */}
        <aside className="w-64 min-w-[220px] hidden md:flex flex-col bg-white/70 dark:bg-slate-900/80 backdrop-blur-xl border border-white/30 dark:border-slate-700 shadow-2xl rounded-2xl p-6 space-y-2 h-fit sticky top-28">
          <div className="flex flex-col items-center mb-6">
            ...
            />
            <span className="font-bold text-lg text-gray-800 dark:text-gray-100">{displayName}</span>
          ...
          {sidebarRoutes.map((route) => (
            <Link
              ...
              className={`flex items-center px-4 py-2 rounded-lg font-medium transition-all duration-200 ${location.pathname === route.path
                  ...
                  : "text-gray-800 dark:text-gray-200 hover:bg-white/80 dark:hover:bg-slate-800 hover:shadow-md hover:text-green-700 dark:hover:text-green-400"
                }`}
            >
              {route.icon}
              {route.name}
        ...
        {isSidebarOpen && (
          <div className="fixed inset-0 bg-black/40 z-50 flex">
            <div className={`
              bg-white dark:bg-slate-900 w-64 p-6 flex flex-col space-y-4 h-full
              transform transition-transform duration-300 ease-in-out
              ${isSidebarOpen ? "translate-x-0" : "-translate-x-64"}
            `}>
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">Dashboard</h3>
                <button onClick={() => setIsSidebarOpen(false)} className="p-2 bg-gray-100 dark:bg-slate-700 rounded-full">
              ...
              <div className="flex flex-col items-center mb-4">
                ...
                />
                <span className="font-bold text-base text-gray-800 dark:text-gray-100">{displayName}</span>
                <span className="text-xs text-gray-500 dark:text-gray-400">{displayEmail}</span>
              </div>
              {sidebarRoutes.map((route) => (
                <Link
        ...
        {/* Main Content */}
        <main className="flex-1 bg-white/60 dark:bg-slate-900/70 backdrop-blur-lg border border-white/30 dark:border-slate-700 shadow-2xl rounded-2xl p-6 min-h-[400px]">
